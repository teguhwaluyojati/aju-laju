...
export default function RegisterPage() {
  ...
  // Do not block authenticated-but-unverified users because they need to see the verification modal.
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (credential.user && !credential.user.emailVerified) {
        ...
        setRegisteredEmail(credential.user.email || email.trim());
      ...
      >
        <div className="space-y-4">
          <p className="text-sm leading-relaxed text-ink-muted">
            {t(
              "Registrasi berhasil. Kami sudah mengirim link aktivasi ke email berikut:",
              "Registration successful. We have sent an activation link to:"
            )}
          </p>
          <p className="rounded-lg bg-slate-50 px-3 py-2 text-sm font-medium text-ink">{registeredEmail || user?.email || "-"}</p>
          <p className="text-xs text-ink-subtle">
            {t(
              "Cek inbox/spam, klik link verifikasi, lalu lanjutkan login.",
              "Check inbox/spam, click the verification link, then continue."
            )}
          </p>
